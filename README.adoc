...
Relaton::Render::General.new(template: ..., nametemplate: ..., seriestemplate: ..., language: "en", script: "Latn", input: "relaton").render(input)
----
...
Relaton Render by default processed Relaton XML. The format of documents it processes is specified with `input`,
and can be either `XML` (Relaton XML) or `Relaton` (Relaton classes, native format of Relaton gems).
